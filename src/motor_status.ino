...
inline bool isMultiAngle92Cmd(uint8_t c) { return c == 0x92; }
...
// Multi-turn (0x92) cache (raw 0.01° LSB)
static int64_t  g_multi_0p01    [256];  // signed cumulative angle in 0.01°
static bool     g_have_multi_92 [256];  // whether we have a fresh sample
static uint32_t g_multi_stamp_ms[256];  // receipt timestamp
...
void sendRead92(uint8_t id){
  CAN_message_t m{}; m.id = status_motor_can_id(id); m.len = 8;
  for (int i=0;i<8;++i) m.buf[i]=0;
  m.buf[0] = 0x92;  // multi-turn absolute angle (0.01°/LSB), signed int64 reply
  Can1.write(m);
}
...
static inline void parseAngle92FromFrame(const CAN_message_t& rx){
  const uint8_t id = static_cast<uint8_t>(rx.id - 0x140u);
  // DATA[0] = 0x92, DATA[1..7] = little-endian int64 in 0.01°/LSB
  uint64_t u = 0;
  u |= (uint64_t)rx.buf[1];
  u |= (uint64_t)rx.buf[2] << 8;
  u |= (uint64_t)rx.buf[3] << 16;
  u |= (uint64_t)rx.buf[4] << 24;
  u |= (uint64_t)rx.buf[5] << 32;
  u |= (uint64_t)rx.buf[6] << 40;
  u |= (uint64_t)rx.buf[7] << 48;
  int64_t s = (int64_t)u;  // interpret as signed

  g_multi_0p01[id]     = s;
  g_have_multi_92[id]  = true;
  g_multi_stamp_ms[id] = millis();
}
...
void poll(){
  ...
  while (Can1.read(rx)) {
    ...
    else if (isMultiAngle92Cmd(cmd)) parseAngle92FromFrame(rx);
...
// ---- Multi-turn 0x92 getters (to be declared in motor_status.h later) ----
bool getMultiAngle92Raw(uint8_t id, int64_t* ang_0p01){
  if (!ang_0p01) return false;
  if (!g_have_multi_92[id]) return false;
  *ang_0p01 = g_multi_0p01[id];
  return true;
}

bool getMultiAngle92Deg(uint8_t id, float* deg){
  if (!deg) return false;
  if (!g_have_multi_92[id]) return false;
  *deg = (float)g_multi_0p01[id] * 0.01f;
  return true;
}
